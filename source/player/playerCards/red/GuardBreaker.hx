 ...
 * @author Joshua ellis
 */
class GuardBreaker extends PlayerCard
{
	...
	{
		if (target.effects.maxGuard() > 0){
			target.damageNumbers.addNumber('Guard Break '+Std.string(Std.int(Math.min(target.effects.maxGuard(),16))),FlxColor.RED);
		}
		target.effects.removeGuard(16);
		damage(target, 5);
